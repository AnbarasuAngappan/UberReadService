...
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "UberService" in both code and config file together.
    public class UberService : IUberService
    {
        public DataTable GetHousdetails(string _societyID, string _apiKey)
        {
            ...
            using (SqlCommand cmd = new SqlCommand())
                try
                {
                    using (cmd)
                    {
                        ...
                        cmd.Parameters.Add("@APIID", _apiKey);
